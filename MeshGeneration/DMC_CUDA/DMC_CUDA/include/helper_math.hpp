...
inline __host__ __device__ void normalize(float2& v)
{
    float invLen = rsqrtf(dot(v, v));
    v *= invLen;
}

inline __host__ __device__ float2 normalized(float2 v)
{
	normalize(v);
	return v;
    /*
...
inline __host__ __device__ float4 normalized(float4 v)
{
    float invLen = rsqrtf(dot(v, v));
    return v * invLen;
}
