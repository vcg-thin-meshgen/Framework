...
template <typename T> struct tup2 { T x, y; };
template <typename T> struct tup3 { T x, y, z; };
template <typename T> struct tup4 { T x, y, z, w; };

using float2 = tup2<float>;
using float3 = tup3<float>;
using float4 = tup4<float>;

using int2 = tup2<int>;
using int3 = tup3<int>;
using int4 = tup4<int>;

using uint2 = tup2<uint>;
using uint3 = tup3<uint>;
using uint4 = tup4<uint>;
...
inline void normalize(float2& v)
{
    float invLen = rsqrtf(dot(v, v));
    v *= invLen;
}

inline float2 normalized(float2 v)
{
	normalize(v);
	return v;
    /*
...
inline float4 normalized(float4 v)
{
    float invLen = rsqrtf(dot(v, v));
    return v * invLen;
}
